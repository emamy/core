function [Y,E] = PlotParamSweep(rmodel, mu, inputidx, param, paramvals)
% PlotParamSweep: Plots the output with error bounds for a range of one specified parameter.
%
...
% Perform first simulation and check one-dimensionality
mu(pidx) = paramvals(1);
...
% % surfl(T,MU,Y);
% xlabel('t'); ylabel('\mu'); axis tight; title(tit);
