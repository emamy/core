classdef MatlabDocMaker
    ...
    %
    % @change{0,5,dw,2011-11-04} Changed the name to MatlabDocMaker in
    ...
    methods(Static)
        ...
        function version = getProjectVersion
            ...
            version = [KerMor.MainVersion '.' KerMor.SubVersion];
        end
    end
    ...
    methods(Static)
        ...
        function setup
            ...
            if isempty(doxybin)
                [st, doxybin] = system('which doxygen');
                if st == 0
                    doxybin = doxybin(1:end-1); % remove \n char
                    word = sprintf('Doxygen found at %sDo you want to specify a custom doxygen binary?\n(Y)es/(N)o?: ',doxybin);
                else
