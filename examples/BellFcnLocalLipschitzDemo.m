...
%         end
    ...
    plot(x,f(x),'r',x,abs(df(x)),'r--',x,e1,'b',x,e2,'g',x,e3,'m');
    hold on;
    %plot(x,e12,'b--',x,e22,'g--',x,e32,'m--');
    title(sprintf('C=%f',C));
    
    legend('f(x)','Df(x)','Error 1','Error 2','Error 3');
    
    skip = round(1/(3*dt));
    plot(x(1:skip:end),minder(1:skip:end),'rx');
    %plot(x,minder,'r');
    legend('f(x)','Df(x)','Error 1','Error 2','Error 3','Minimum');
